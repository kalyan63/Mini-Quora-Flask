...
GetUserDetails,GetMailQuestion,GetUserQues,GetUserAns,GetFollowId,InsertQues,GetQuestion,GetAnswer,InsertAns,\
...
def Question(Q_id):
    if(request.method=="POST"):
        ...
        if(A_txt):
            ...
            Own_mail=GetMailQuestion(Q_id)   
            try:
                msg = Message( 'Hello from mini-quora!', sender ='chiluverupreeti@gmail.com', recipients = [str(Own_mail)])
                msg.body = 'Hello!! You got a message from Mini-quora! Your Posted question has been answered'
                mail.send(msg) 
            except:
                print("Invalid user mail")    
        else:
